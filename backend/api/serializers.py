...
class StorySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Story
        fields = ['id', 'user', 'media_url', 'media_type', 'created_at', 'expires_at', 'is_viewed']
    def get_is_viewed(self, obj):
        request = self.context.get('request')
        return request and request.user.is_authenticated and StoryView.objects.filter(story=obj, viewer=request.user).exists()
